...
<template>
    <AuthenticatedLayout>
        <Head :title="loading ? 'Editar Produto' : `Editar: ${form.description}`" />
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <div v-show="activeTab === 'geral'" class="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6">
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
                            <draggable v-model="form.existing_images" item-key="id" class="contents" v-bind="dragOptions">
                                <template #item="{ element, index }">
                                    <div class="relative group aspect-square rounded-2xl overflow-hidden border border-gray-100 shadow-sm cursor-move">
                                        ...
                                        <span class="absolute top-2 left-2 bg-black/50 backdrop-blur-md text-[8px] text-white px-2 py-0.5 rounded-full uppercase font-black">{{ index + 1 }}o</span>
                                    </div>
                                </template>
                            </draggable>
                ...
                <div v-show="activeTab === 'precos'" class="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2 space-y-6">
                            ...
                            <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="flex items-center text-xs font-black uppercase text-gray-400 italic"><Truck class="w-4 h-4 mr-2" /> Logistica</h3>
                        ...
                        <div class="bg-indigo-900 p-8 rounded-3xl text-white h-fit">
                            <h3 class="text-[10px] font-black uppercase opacity-60 mb-6 italic">Promocao</h3>
                            <div class="space-y-4">
                                <div><label class="text-[9px] opacity-40 block mb-1">Preco Promo</label><input v-model="form.promo_price" type="number" step="0.01" class="w-full bg-indigo-800 border-none rounded-2xl text-white font-bold" /></div>
                                <div><label class="text-[9px] opacity-40 block mb-1">Inicio</label><input v-model="form.promo_start_at" type="datetime-local" class="w-full bg-indigo-800 border-none rounded-xl text-[10px] text-white" /></div>
                                <div><label class="text-[9px] opacity-40 block mb-1">Termino</label><input v-model="form.promo_end_at" type="datetime-local" class="w-full bg-indigo-800 border-none rounded-xl text-[10px] text-white" /></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-show="activeTab === 'seo'" class="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-6">
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        <div class="flex items-center gap-3"><Code class="w-5 h-5 text-indigo-600" /><h3 class="text-xs font-black uppercase tracking-widest text-gray-500">Configuracoes de URL</h3></div>
                        <div><label class="block text-[10px] font-black uppercase text-gray-400 mb-2">Slug da URL (Canonica)</label><input v-model="form.slug" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold text-xs" /><p class="mt-2 text-[10px] text-gray-400 italic">Cuidado: alterar o slug pode quebrar links externos indexados.</p></div>
                    ...
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        ...
                        <div><label class="block text-[10px] font-black uppercase text-gray-400 mb-2">SEO Title (Max 70)</label><input v-model="form.meta_title" maxlength="70" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" required /></div>
                        <div>
                        ...
                        <div><label class="block text-[10px] font-black uppercase text-gray-400 mb-2">Meta Description (Max 160)</label><textarea v-model="form.meta_description" maxlength="160" rows="2" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold text-xs" required></textarea></div>
                    </div>
                    ...
                    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label class="block text-[10px] font-black uppercase text-gray-400 mb-2">Titulo H1 Principal</label><input v-model="form.h1" type="text" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" required /></div>
                            <div><label class="block text-[10px] font-black uppercase text-gray-400 mb-2">Texto de Apresentacao (Text 1)</label><textarea v-model="form.text1" rows="3" class="w-full border-gray-100 bg-gray-50 rounded-2xl font-bold" required></textarea></div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
