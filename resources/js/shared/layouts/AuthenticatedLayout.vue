...
<template>
    <div class="min-h-screen bg-green-50 flex overflow-x-hidden font-sans text-slate-900">
        <Transition
        ...
        >
            <div
                ...
                class="fixed inset-0 bg-green-950/60 z-40 md:hidden backdrop-blur-sm"
            />
        </Transition>

        <aside :class="[
            'fixed inset-y-0 left-0 w-64 bg-green-950 text-white flex flex-col z-50 transition-transform duration-300 ease-in-out md:translate-x-0 shadow-2xl',
            isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
        ]">
            <div class="p-6 border-b border-green-900/50 flex justify-between items-center">
                <span class="font-black text-xl tracking-tighter">ERP<span class="text-green-400">API</span></span>
                <button @click="isMobileMenuOpen = false" class="md:hidden p-1 hover:bg-green-900 rounded-lg transition-colors">
                    <X class="w-6 h-6" />
                </button>
            </div>
            ...
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto scrollbar-none [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-smooth">
                <Link
                    ...
                    :class="[isUrl('/dashboard') ? 'bg-green-600 text-white shadow-lg shadow-green-600/30' : ' hover:bg-green-900 hover:text-white']"
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest text-green-400/50 mt-6 mb-2 px-3">Comercial</p>
                ...
                <p class="text-[10px] font-bold uppercase tracking-widest text-green-400/50 mt-6 mb-2 px-3">Logística</p>
                <Link
                    :href="route('products.index')"
                    :class="[isUrl('/products') ? 'bg-green-600 text-white shadow-lg shadow-green-600/30' : ' hover:bg-green-900 hover:text-white']"
                    class="flex items-center gap-3 p-3 rounded-xl font-medium transition-all"
                >
                    <Package class="w-5" /> <span>Produtos</span>
                </Link>

                <p class="text-[10px] font-bold uppercase tracking-widest text-green-400/50 mt-6 mb-2 px-3">Gestão</p>
                <Link
                    :href="route('users.index')"
                    :class="[isUrl('/users') ? 'bg-green-600 text-white shadow-lg shadow-green-600/30' : ' hover:bg-green-900 hover:text-white']"
                    class="flex items-center gap-3 p-3 rounded-xl font-medium transition-all"
                >
                ...
                <div class="pt-2">
                    <button
                        ...
                        class="flex items-center justify-between w-full p-3 rounded-xl hover:bg-green-900 hover:text-white transition-all font-medium"
                    >
                    ...
                    >
                        <div v-if="showReportsMenu" class="ml-11 mt-1 space-y-1">
                            <Link :href="route('reports.index')" class="block p-2 text-sm text-green-400 hover:text-white transition-colors">Produtos</Link>
                            <span class="block p-2 text-sm text-green-800 cursor-not-allowed italic">Vendas</span>
                        </div>
                    </Transition>
                </div>
            </nav>

            <div class="p-4 border-t border-green-900/50">
                <Link
                    :href="route('logout')"
                    method="post"
                    as="button"
                    class="flex items-center gap-3 w-full p-3 text-green-300 hover:text-white hover:bg-orange-900 rounded-xl transition-all font-medium cursor-pointer"
                >
        ...
        <div class="flex-1 md:ml-64 flex flex-col min-w-0">
            <header class="h-16 bg-green-900 border-b border-green-100 flex items-center justify-between px-6 sticky top-0 z-30 shadow-sm">
                <div class="flex items-center gap-4">
                    <button @click="toggleMobileMenu" class="md:hidden p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors">
                        <Menu class="w-6 h-6" />
                    </button>
                    <h2 class="hidden md:block text-xs font-bold text-green-300 uppercase tracking-widest text-white">Painel de Controle</h2>
                </div>

                <div class="flex items-center gap-4">
                    <div class="text-right hidden sm:block">
                        <p class="text-xs font-bold text-slate-900 leading-none mb-1 text-white">{{ user.name }}</p>
                        <p class="text-[9px] text-green-500 font-black uppercase tracking-tighter bg-green-50 px-2 py-0.5 rounded-md inline-block">Admin</p>
                    </div>
                    <div class="w-10 h-10 rounded-xl bg-green-600 flex items-center justify-center text-white font-black shadow-lg shadow-green-600/20 ring-4 ring-green-50">
                        {{ user.name.charAt(0) }}
                    </div>
                </div>
            </header>
        ...
        >
            <div v-if="showToast" class="fixed bottom-8 right-8 z-[100] w-full max-w-sm px-4 sm:px-0">
                <div :class="[
                    ...
                    toastType === 'success' ? 'bg-white/90 border-green-200 shadow-green-950/5' : 'bg-white/90 border-green-200 shadow-green-950/5'
                ]">
                    <div :class="['p-2.5 rounded-2xl shrink-0 shadow-lg', toastType === 'success' ? 'bg-green-500 shadow-green-500/40' : 'bg-green-500 shadow-green-500/40']">
